...
<x-staff-layout>
    ...
    <script>
        function staffProfileEditor() {
            return {
                ...
                phoneDraft: @json($technicalStaff ? ($technicalStaff->phone ?? '') : ''),
                locationDraft: @json($technicalStaff ? ($technicalStaff->location ?? '') : ''),
                ...
                cancelEdit() {
                    ...
                    this.phoneDraft = @json($technicalStaff ? ($technicalStaff->phone ?? '') : '');
                    this.locationDraft = @json($technicalStaff ? ($technicalStaff->location ?? '') : '');
                },
                ...
                async submitField(field) {
                    ...
                    else if (field === 'location') { value = this.locationDraft ?? ''; key = 'location'; }
                    else { return true; }
    ...
    <div class="py-2" x-data="staffProfileEditor()">
                ...
                <div class="order-2 lg:col-span-2 lg:order-1 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <dl class="divide-y divide-gray-100">
                        ...
                        <div class="flex items-center gap-4 px-6 py-4">
                            ...
                            <dt class="w-40 text-sm font-medium text-gray-500">Location</dt>
                            <template x-if="!isEditing">
                                <dd class="flex-1 text-sm font-medium text-gray-900" x-text="locationDraft || '—'"></dd>
                            </template>
                            <template x-if="isEditing">
                                <dd class="flex-1">
                        ...
                        <div class="flex items-center gap-4 px-6 py-4">
                            ...
                            <dd class="flex-1 text-sm font-medium text-gray-900">{{ $roleLabel }}</dd>
                        </div>
                        ...
                        <div class="flex items-center gap-4 px-6 py-4">
                            ...
                            <dd class="flex-1 text-sm font-medium text-gray-900">{{ $user->created_at ? $user->created_at->format('F d, Y') : '—' }}</dd>
                        </div>
                    </dl>
                </div>

                {{-- Right column: profile summary card --}}
                <div class="order-1 lg:order-2 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6 flex flex-col items-center">
                    
                    {{-- Avatar with status badge --}}
                    <div class="relative inline-flex items-center justify-center">
                        <div class="relative flex h-24 w-24 items-center justify-center rounded-full bg-blue-600 text-2xl font-bold text-white shadow-md overflow-hidden">
                        ...
                        <span class="absolute bottom-1 right-1 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 border-2 border-white">
                    ...
                    <div class="mt-4 text-center">
                        <h2 class="text-xl font-bold text-slate-900" x-text="nameDraft"></h2>
                        <span class="mt-1 inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-sm font-semibold text-blue-700">
                    ...
                    {{-- Contact details block (Naka-center ang kabuuan pero naka-align sa unang letra ang icons & text) --}}
                    <div class="mt-6 mx-auto w-fit space-y-3 text-left">
                        <!-- Email Row -->
                        <div class="flex items-center gap-3 text-sm font-medium text-slate-700">
                            ...
                            <span class="truncate" x-text="emailDraft"></span>
                        </div>
                        ...
                        <div class="flex items-center gap-3 text-sm font-medium text-slate-700">
                            ...
                            <span class="truncate" x-text="phoneDraft || '—'"></span>
                        </div>
                        ...
                        <div class="flex items-center gap-3 text-sm font-medium text-slate-700">
                            ...
                            <span class="truncate" x-text="locationDraft || '—'"></span>
                        </div>
                    </div>
            ...
            <div class="mt-6 bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center">
                        <div class="bg-blue-50 text-blue-600 rounded-lg p-2.5 mr-3">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                            </svg>
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-slate-900">My Schedule</h3>
                            <p class="text-xs text-slate-500 mt-0.5">View your work schedule and shifts.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2.5">
                    @forelse ($scheduleDays ?? [] as $day)
                        <div class="bg-slate-50/70 border border-slate-100 rounded-xl px-5 py-3.5 flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-800 w-36">{{ $day->date->format('D, M j') }}</span>
                            <span class="text-xs font-bold text-slate-800 flex-1 text-center">{{ $day->start_time }} - {{ $day->end_time }}</span>
                            <span class="bg-emerald-100/80 text-emerald-700 text-[11px] font-semibold px-3 py-1 rounded-full">
                                {{ $day->shift_type === 'night' ? 'Night Shift' : 'Day Shift' }}
                            </span>
                        </div>
                    @empty
                        <div class="bg-slate-50/70 border border-slate-100 rounded-xl px-5 py-3.5 flex items-center justify-center text-xs text-slate-500">
                ...
                <div class="mt-4 flex items-center text-xs font-medium text-slate-500">
                    <img src="{{ asset('image/Staff/clock-circle-svgrepo-com.svg') }}" alt="Clock" class="h-4 w-4 text-slate-400 mr-1.5">
